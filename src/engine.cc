...
img::EasyImage generate_image(const ini::Configuration &configuration) {
    ...
    if (type == "2DLSystem") {
        ...
        image.drawLSystem(l_system, size, scale_colors_(color));
    }
        ...
        for (Figure & i : figures) {
            i.apply_transformation(Figure::eye_point_trans(Vector3D::point(eye[0], eye[1], eye[2])));
            Lines2D figure_lines = i.do_projection();
            figures_lines.splice(figures_lines.end(), figure_lines);
        }

        image.draw2DLines(figures_lines, size);
